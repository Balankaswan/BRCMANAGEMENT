...
const FuelManagement: React.FC = () => {
  ...
  const handleFuelAllocation = async () => {
    ...
    if (selectedMemo && selectedVehicle) {
      alert('Please select either a Vehicle OR a Memo, not both');
      return;
    }
    ...
    try {
      ...
      // If a memo is selected, record the fuel as an advance payment on the memo
      if (selectedMemo) {
        ...
        if (memo) {
          try {
            ...
            console.log('✅ Fuel advance recorded on memo:', memo.memo_number);
          } catch (memoError) {
            console.error('❌ Failed to update memo advance:', memoError);
          }
        }
      }
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Memo Number (Optional)</label>
                <select
                  value={selectedMemo}
                  onChange={(e) => {
